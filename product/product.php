<?php
  ...
  class Product{
    ...
    public function insert_product(){
      ...
      $this->pr_img   = "https://ckgpwl2.cafe24.com/baexang_front/images/".$this->table.'/'.htmlspecialchars(strip_tags($this->pr_img)); 
      ...
      $this->pr_ID    = mt_rand(1, 1000000); // 1~백만 사이 랜덤 숫자
      $this->pr_hit   = 0;
    ...
    public function get_products(){
      if($this->cate == 'all'){ // 테이블이 모두 선택될때
        if($this->sort == 'new'){
          ...
          $sql = "SELECT * FROM (SELECT * FROM bx_pp UNION SELECT * FROM bx_dp) AS union_table".$this->pr_ID.$this->limit; // 서브쿼리 : union은 컬럼명과 갯수가 같아야 한다. 
        }
        // echo $sql;
      } else { // 지정된 테이블만 조회될때
        if($this->sort == 'new'){
