<?php
  ...
  $img_t_size       = $_FILES['pr_img']['size']; // 174119
  $img_n_start_name = $ext_n_arr[0]; // new-art-3
  $img_n_end_name   = end($ext_n_arr); // jpg
  $img_t_end_name   = end($ext_t_arr); // phpoVE0CX
  ...
  // 이미지 사이즈 제한
  if($img_t_size > 3000000){ // 3MB
    $msg = ['msg' => '파일 용량이 3MB를 초과했습니다.'];
  } else if(!in_array($img_n_end_name, $allowed_exp)){ // $allowed_exp 배열 안에 $img_n_end_name문자열이 없다면 true
