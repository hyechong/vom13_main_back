<?php
  ...
  $data = json_decode(file_get_contents('php://input')); // input을 통해 들어온 데이터 파일을 생성 후 json으로 디코딩
  // print_r($data);
  // echo $data->user_id;
  // exit; 
  // print_r($data);
  ...
  // exit;
  if($signin->login()==0){ // id 가 틀림
    $msg = ['msg' => '존재하지 않는 아이디 입니다.'];
  } else if($signin->login()==1){ // pwd 가 틀림
    $msg = ['msg' => '비밀번호가 맞지 않습니다.'];
  } else { // 다 맞음
